...
# تفعيل السجلات (Logging) لمعرفة سبب أي توقف في منصة Render
logging.basicConfig(
...
# ---------------------------------------------------------
# 1. خادم Flask للحفاظ على تشغيل البوت على Render
# ---------------------------------------------------------
app = Flask("")
...
def keep_alive():
  # إضافة daemon=True يضمن إغلاق السيرفر وإعادة التشغيل إذا تعطل البوت
...
async def setcity_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
  msg = "يرجى إرسال رقم ولايتك (1-58) أو اسم الولاية لتحديثها:"
...
async def salat_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
  try:
    ...
    logger.error(f"Error in salat_command: {e}")
...
async def check_and_send_alerts(context: ContextTypes.DEFAULT_TYPE):
  try:
    ...
    logger.error(f"Error in scheduler alerts: {e}")
...
def main():
  ...
  if app_bot.job_queue:
    app_bot.job_queue.run_repeating(check_and_send_alerts, interval=60, first=10)
  else:
    logger.warning("JobQueue is not properly initialized!")
  ...
  logger.info("Bot is running successfully and polling...")
  app_bot.run_polling()

if __name__ == "__main__":
  main()# 2. البيانات والولايات وتخزين المواقيت (Cache)
# ---------------------------------------------------------
BOT_TOKEN = os.environ.get("BOT_TOKEN")
