...
# ---------------------------------------------------------
# 4. حلقة التنبيهات الخلفية (الصلوات + تذكير يوم الجمعة)
# ---------------------------------------------------------
    ...
    try:
      ...
      for chat_id, city_data in list(user_cities.items()):
        ...
        if now.weekday() == 4 and now.hour == 9 and now.minute == 0:
          ...
          if friday_key not in sent_alerts:
            sent_alerts.add(friday_key)
            friday_msg = (
                "🌿 **تذكير يوم الجمعة المباركة** 🕌\n\n"
                "قال الله تعالى: ﴿إِنَّ اللَّهَ وَمَلَائِكَتَهُ يُصَلُّونَ"
                " عَلَى النَّبِيِّ يَا أَيُّهَا الَّذِينَ آمَنُوا صَلُّوا"
                " عَلَيْهِ وَسَلِّمُوا تَسْلِيمًا﴾\n\n"
                "✨ **سنن مستحبة في هذا اليوم:**\n"
                "📖 **قراءة سورة الكهف:** نور ما بين الجمعتين.\n"
                "📖 **قراءة سورة البقرة:** بركة وطرد للشياطين من البيت.\n"
                "🤲 **كثرة الصلاة على النبي ﷺ.**\n"
                "🤲 **تحري ساعة الاستجابة.**\n\n"
                "جمعة مباركة وطيبة عليكم جميعاً 🤍"
            )
            await app_bot.bot.send_message(
                chat_id=chat_id, text=friday_msg, parse_mode="Markdown"
            )

        # --- ب) تنبيهات أوقات الصلاة ---
        prayer_times = fetch_prayer_times(city_data["en"])
        prayers_check = {
        ...
        for prayer_name, time_str in prayers_check.items():
          ...
          # 1. التنبيه المسبق (قبل 10 دقائق)
          pre_alert_key = f"{chat_id}_{today_str}_{prayer_name}_pre"
          if 540 <= diff_seconds <= 600 and pre_alert_key not in sent_alerts:
          ...
          # 2. التنبيه الفوري لحظة الأذان
          now_alert_key = f"{chat_id}_{today_str}_{prayer_name}_now"
          if -30 <= diff_seconds <= 30 and now_alert_key not in sent_alerts:
            sent_alerts.add(now_alert_key)
            alert_msg = (
                f"📢 **حان الآن وقت صلاة {prayer_name} بتوقيت ولاية"
                f" {city_data['ar']} وماجاورها** 🕌\n\n"
                "تقبل الله صلاتكم وطاعاتكم.\n\n"
                "🤲 **دعاء ما بعد الأذان:**\n"
                "«اللهم رب هذه الدعوة التامة والصلاة القائمة، آت محمداً"
...
if __name__ == "__main__":
  main()  port = int(os.environ.get("PORT", 8080))
  app.run(host="0.0.0.0", port=port)
